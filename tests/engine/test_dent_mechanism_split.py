...
from engine.prowrap_mechanisms import MECHANISM_CHOICES, normalize_mechanism
...
def test_component_pipe_allowable_basis_uses_approved_equation():
    ...
    assert basis["allowable_stress_mpa"] == pytest.approx(258.48)
    assert basis["allowable_pressure_mpa"] == pytest.approx(
        10.775653543307088
    )
...
def test_external_dent_no_crack_uses_component_pipe_load_sharing():
    ...
    assert result["allowable_pipe_stress_mpa"] == pytest.approx(258.48)
    assert result["p_steel_capacity"] == pytest.approx(10.775653543307088)
...
def test_dent_routes_without_eligible_substrate_are_type_b(
    ...
    assert result["calculation_basis"] == "Type B full replacement"
    assert result["p_steel_capacity"] == 0.0
    assert result["allowable_pipe_stress_mpa"] is None
...
def test_dent_no_crack_formula5_receives_only_the_pressure_deficit():
    ...
    assert result["p_steel_capacity"] == pytest.approx(3.392125984251969)
    assert result["p_composite_design"] == pytest.approx(8.607874015748031)
